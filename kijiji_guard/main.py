...
def get_version() -> str:
    try:
        return importlib.metadata.version("kijiji-guard")
...
def callback(
    ...
    if version:
        ver = get_version()
        console.print(f"[bold green]Kijiji-Guard[/bold green] v{ver}")
        raise typer.Exit()
...
def watch(
    ...
    """
    from kijiji_guard.core.watcher import KijijiWatcher
