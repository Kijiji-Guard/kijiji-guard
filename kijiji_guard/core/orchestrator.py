...
class KijijiOrchestrator:
    ...
    def _run_scanner(self, scan_type: str, target: str, country: str,
        ...
        try:
            if scan_type == "iac":
                from kijiji_guard.adapters.iac.hcl_adapter import HCLAdapter
                return HCLAdapter().scan(target, country)

            if scan_type == "paas:vercel":
                from kijiji_guard.adapters.paas.vercel_adapter import VercelAdapter
                adapter = VercelAdapter()
                adapter.authenticate(credentials)
                return adapter.scan(country)

            if scan_type == "paas:supabase":
                from kijiji_guard.adapters.paas.supabase_adapter import SupabaseAdapter
                adapter = SupabaseAdapter()
                adapter.authenticate(credentials)
                return adapter.scan(country)

            if scan_type == "paas:firebase":
                from kijiji_guard.adapters.paas.firebase_adapter import FirebaseAdapter
                adapter = FirebaseAdapter()
                adapter.authenticate(credentials)
                return adapter.scan(country)

            if scan_type == "paas:render":
                from kijiji_guard.adapters.paas.render_adapter import RenderAdapter
                adapter = RenderAdapter()
                adapter.authenticate(credentials)
                return adapter.scan(country)

            if scan_type == "paas:railway":
                from kijiji_guard.adapters.paas.railway_adapter import RailwayAdapter
                adapter = RailwayAdapter()
                adapter.authenticate(credentials)
                return adapter.scan(country)

            if scan_type == "api:aws":
                from kijiji_guard.adapters.api.aws_adapter import AWSAdapter
                adapter = AWSAdapter()
                adapter.authenticate(credentials)
                return adapter.scan(country)

            if scan_type == "api:gcp":
                from kijiji_guard.adapters.api.gcp_adapter import GCPAdapter
                adapter = GCPAdapter()
                adapter.authenticate(credentials)
                return adapter.scan(country)

            if scan_type == "api:azure":
                from kijiji_guard.adapters.api.azure_adapter import AzureAdapter
                adapter = AzureAdapter()
                adapter.authenticate(credentials)
                return adapter.scan(country)

            if scan_type == "api:digitalocean":
                from kijiji_guard.adapters.api.digitalocean_adapter import DigitalOceanAdapter
